...
export async function POST(req: Request, { params }: { params: { orgId: string } }) {
  ...
  const myRole = org.members?.find((m: { userId: unknown; role?: string }) => String(m.userId) === String(me._id))?.role || 'editor';
  ...
  const inv = await Invite.create({ orgId: params.orgId, email, role: (requestedRole as any) || 'editor', token, expiresAt, status: 'pending' });
