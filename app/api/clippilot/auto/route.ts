...
import { generateVoiceScript, synthesizeVoice } from "@/lib/clippilot/voice";
  ...
  try {
    ...
    if (wantsVoice) {
      if (body.voiceScript && body.voiceScript.trim().length > 0) {
        ...
        finalVoiceScript = await generateVoiceScript(plan.summary, transcript);
      }
    }
    ...
    if (finalVoiceScript && finalVoiceScript.trim().length > 0) {
      voicePath = await synthesizeVoice(finalVoiceScript);
    }
