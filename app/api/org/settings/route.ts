...
export async function GET() {
  ...
  await dbConnect()
  const me = await User.findOne({ email: session.user.email }).lean<{ _id: mongoose.Types.ObjectId; orgId?: mongoose.Types.ObjectId | string; role?: string }>()
...
export async function PATCH(req: Request) {
  ...
  if (!['owner','manager'].includes(myRole)) return new Response('Forbidden', { status: 403 })
  ...
  // Dev-only plan override: allowed when not production and either env flag enabled or owner/manager
  const allowByRole = myRole === 'manager' || myRole === 'owner'
