...
export async function generateContent({ orgId, userId, module, prompt, vars = {}, jsonSchema }:{ orgId:string, userId:string, module:'post'|'blog'|'ad'|'lead'|'mail', prompt?:string, vars?:Record<string,any>, jsonSchema?:any }){
  await dbConnect()
  const limits:Record<string,number>={ post:60, blog:20, ad:60, lead:60, mail:60 }
  const rl = await checkRateLimit(orgId, module, limits[module]||30, 60)
  if(!rl.ok){ return { ok:false, error:'rate_limited', remaining:0 } }

  const usedPrompt = prompt || ''; const usedSchema = jsonSchema
  ...
  const gen = await Generation.create({ orgId, userId, module, input: { vars, prompt: usedPrompt }, status: 'running', events:[{type:'requested', at:new Date(), meta:{}}], safety:{ok:true, reasons:[]} })
